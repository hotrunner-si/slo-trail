...
const collection = ref('vse'), view = ref<ViewMode>('map'), density = ref<'comfortable' | 'compact'>('comfortable')
  ...
  <div class="page races-page">
    <section class="race-explorer shell" aria-label="Raziskovalnik tekem">
      <div class="race-search">
        <div><span>⌕</span><input id="race-search" v-model="query" type="search"
            placeholder="Išči dogodek, kraj, razdaljo, ..."><kbd>/</kbd></div>
      </div>
      <div class="race-collections" aria-label="Uredniške zbirke"><button v-for="item in collections" :key="item.id"
          :class="{ active: collection === item.id }" @click="collection = item.id"><strong>{{ item.label }}</strong><span>{{
            item.description }}</span></button></div>
      <details class="advanced-filters">
        <summary>Filtri <span class="mono">{{ filtered.length }} REZULTATOV</span></summary>
        <div class="filters filters-seven"><label>Mesec<select v-model="month">
              <option v-for="item in months" :key="item">{{ item }}</option>
            </select></label><label>Razdalja<select v-model="distance">
              <option>Vse razdalje</option>
              <option>Do 30 km</option>
              <option>30–60 km</option>
              <option>Nad 60 km</option>
            </select></label><label>Regija<select v-model="region">
              <option>Vse regije</option>
              <option v-for="item in [...new Set(races.map(r => r.region))]" :key="item">{{ item }}</option>
            </select></label><label>Tip<select v-model="raceType">
              <option>Vsi tipi</option>
              <option>Trail</option>
              <option>Gorski tek</option>
              <option>Vertikal</option>
            </select></label><label>Effort<select v-model="effort">
              ...
              <option>100K</option>
              <option>100M</option>
            </select></label><label>Tehničnost<select v-model="technicality">
              <option>Vsa zahtevnost</option>
              <option>Tekoča</option>
              <option>Srednja</option>
              <option>Tehnična</option>
            </select></label><label>Prijave<select v-model="status">
      ...
      <div class="race-toolbar">
        <div class="view-switch" aria-label="Način prikaza"><button :class="{ active: view === 'list' }"
            @click="view = 'list'">Seznam</button><button :class="{ active: view === 'map' }"
            @click="view = 'map'">Zemljevid</button><button :class="{ active: view === 'calendar' }"
            @click="view = 'calendar'">Koledar</button></div>
        <div class="toolbar-right">
          <div class="effort-legend"><span class="effort-short">&lt;20</span><span class="effort-20k">20K</span><span
              class="effort-50k">50K</span><span class="effort-100k">100K</span><span
              class="effort-100m">100M</span><em>KM-EFFORT</em></div><button v-if="view === 'list'" class="density-toggle"
            @click="density = density === 'compact' ? 'comfortable' : 'compact'">{{ density === 'compact' ? 'Razširjeno' :
            'Strnjeno' }}</button>
        </div>
      </div>
      <template v-if="filtered.length">
        <RaceList v-if="view === 'list'" :races="filtered" show-profile show-compare :density="density"
          :active-id="activeId" :compared-keys="comparedKeys" @activate="activeId = $event" @compare="toggleCompare" />
        <RaceCalendar v-else-if="view === 'calendar'" :races="filtered" />
      </template>
      <div v-else class="empty-state">
        <h2>Ni tekem za izbrane pogoje.</h2>
        <p>Poskusi odstraniti enega od filtrov ali ponastavi celoten pogled.</p><button class="button secondary"
          @click="resetFilters">Ponastavi filtre</button>
      </div>
      <RaceMap v-show="view === 'map' && filtered.length > 0" :races="filtered" :active-id="activeId" :visible="view === 'map' && filtered.length > 0" @activate="activeId = $event" />
    </section>
    <section class="data-promise shell">
      ...
      <div>
        <p>Pri vsaki tekmi bomo prikazali datum zadnjega preverjanja, uradni vir in stanje GPX datoteke. Tako bo jasno,
          kateri podatki so potrjeni in kateri še čakajo na osvežitev.</p>
        <dl>
          <div>
            <dt>Preverjeno</dt>
          ...
          <div>
            ...
            <dd>{{races.filter(r => r.distances.some(d => d.gpx)).length}}/{{ races.length }}</dd>
          </div>
          <div>
            <dt>Dogodkov</dt>
            <dd>{{ races.length }}</dd>
