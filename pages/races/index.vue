...
const gpxOnly = ref(false), collection = ref('vse'), view = ref<ViewMode>('list'), density = ref<'comfortable' | 'compact'>('comfortable')
...
const collections = [
  { id:'vse', label:'Vse tekme', description:'44 slovenskih dogodkov' }, { id:'kratke', label:'Kratke trase', description:'Do 20 kilometrov' },
  { id:'srednje', label:'Srednje trase', description:'Od 20 do 60 kilometrov' }, { id:'ultra', label:'Ultra', description:'80 kilometrov in več' },
...
const filtered = computed(() => races.filter(race => {
  ...
  const collectionMatch = collection.value === 'vse'
    || (collection.value === 'kratke' && race.distances.some(d => d.km > 0 && d.km < 20))
    || (collection.value === 'srednje' && race.distances.some(d => d.km >= 20 && d.km < 60))
    || (collection.value === 'ultra' && race.distances.some(d => d.km >= 80))
  ...
  return (!query.value || haystack.includes(query.value.toLowerCase())) && (month.value === 'Vsi meseci' || monthName === month.value.toLowerCase())
    ...
    && (!gpxOnly.value || race.distances.some(d => d.gpx?.file || d.gpx?.profile)) && collectionMatch
}))
...
function resetFilters(){ query.value='';month.value='Vsi meseci';distance.value='Vse razdalje';region.value='Vse regije';status.value='Vsi statusi';raceType.value='Vsi tipi';effort.value='Vsi razredi';technicality.value='Vsa zahtevnost';gpxOnly.value=false;collection.value='vse' }
</script>
...
<template><div class="page races-page">
  <section class="race-explorer shell" aria-label="Raziskovalnik tekem">
    ...
    <template v-if="filtered.length"><RaceList v-if="view==='list'" :races="filtered" show-profile :density="density" :active-id="activeId" :compared-keys="comparedKeys" @activate="activeId=$event" @compare="toggleCompare"/><RaceMap v-else-if="view==='map'" :races="filtered" :active-id="activeId" @activate="activeId=$event"/><RaceCalendar v-else :races="filtered"/></template>
