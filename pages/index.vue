...
useSiteSeo(undefined, 'Trail Slovenija: slovenske trail in gorskotekaške tekme, tekači, GPX ture in zgodbe s poti.', { image: '/images/kv01.jpg', imageAlt: 'Trail tek v slovenskih gorah' })
useHead({ script: [{ type: 'application/ld+json', innerHTML: JSON.stringify({
  '@context': 'https://schema.org', '@type': 'WebSite', name: 'Trail Slovenija',
  url: 'https://trail-slovenija.netlify.app/', inLanguage: 'sl-SI',
...
<template>
  <div>
    ...
    <section v-if="latestArticle" class="section shell home-feature-article"><div><p class="eyebrow">Članek · {{ latestArticle.category }}</p><h2>{{ latestArticle.title }}</h2><p>{{ latestArticle.subtitle }}</p><NuxtLink :to="`/journal/${latestArticle.slug}`" class="button primary">Preberi objavo ↗</NuxtLink></div><img :src="latestArticle.image" :alt="latestArticle.title" loading="lazy"></section>

    <section class="section ink-section"><div class="shell"><SectionHead eyebrow="Rezultati zadnjega vikenda" title="Časi in uvrstitve" link-label="Vsi rezultati" to="/journal"/><NuxtLink v-if="latestResults" :to="`/journal/${latestResults.slug}`" class="home-results-card"><span class="mono">{{ new Date(latestResults.publishedAt).toLocaleDateString('sl-SI') }}</span><div><h3>{{ latestResults.title }}</h3><p>{{ latestResults.description }}</p></div><em>↗</em></NuxtLink><p v-else class="home-results-empty">Pregled rezultatov bo objavljen tukaj.</p></div></section>
    ...
    <section class="section route-week"><div class="shell route-week-grid"><div class="route-photo"><img src="/images/kv01.jpg" alt="Trail tekač na gorski turi" loading="lazy"><RouteVisual :variant="3" dark/></div><div class="route-week-copy"><p class="eyebrow">Tura tedna · kurirano</p><h2>{{ trailRoutes[0].name }}</h2><p>{{ trailRoutes[0].description }}</p><StatStrip :items="[{value:`${trailRoutes[0].distance.toLocaleString('sl-SI')} KM`,label:'Razdalja'},{value:`${trailRoutes[0].elevationGain.toLocaleString('sl-SI')} M+`,label:'Vzpon'},{value:trailRoutes[0].difficulty.toUpperCase(),label:'Težavnost'}]"/><ElevationProfile/><NuxtLink :to="`/routes/${trailRoutes[0].slug}`" class="button primary">Razišči turo ↗</NuxtLink></div></div></section>

    <section id="newsletter" class="section shell newsletter"><div><p class="eyebrow">Novice</p><h2>Trail Weekly</h2><p class="newsletter-lead">Novice in zgodbe iz slovenskega traila.</p></div><div><ul><li>rezultati prejšnjega tedna</li><li>tekme naslednjega vikenda</li><li>trase in tekači</li></ul><form @submit.prevent="subscribe"><label for="email" class="sr-only">E-poštni naslov</label><input id="email" v-model="email" type="email" required placeholder="tvoj@email.si"><button class="button primary" type="submit">Prijavi me ↗</button></form><p v-if="message" class="form-message" role="status">{{ message }}</p></div></section>
  </div>
</template>
