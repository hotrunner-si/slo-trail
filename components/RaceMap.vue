...
const gpxRoutes = computed(() => props.races.flatMap(race => race.distances.map((distance, index) => ({ race, distance, index, data: gpxData.value[`${race.id}:${distance.id || index}`] })).filter(item => item.data?.route?.length)))
const allPoints = computed(() => gpxRoutes.value.flatMap(item => item.data?.route || []))
  ...
  <div class="race-map-layout">
    ...
    <div class="race-map race-map-legacy" role="img" aria-label="Zemljevid lokacij tekem v Sloveniji">
      <svg viewBox="0 0 100 100" aria-hidden="true">
        ...
        <polyline v-for="item in gpxRoutes" :key="`${item.race.id}-${item.distance.id}`" :points="projectRoute(item.data!.route!)" :class="['race-map-gpx-route',`effort-${effortCategory(item.distance).toLowerCase()}`,{active:activeId===item.race.id}]" @mouseenter="emit('activate',item.race.id)"/>
      </svg>
